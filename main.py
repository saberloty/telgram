...
from storage import load_users_data, save_users_data
...
async def start(message: Message, state: FSMContext):
    ...
    if user_id in users_data:
        await message.answer(f"سلام {message.from_user.first_name} عزیز 🌟\nشما قبلاً ثبت‌نام کرده‌اید و می‌توانید عکس یا کلیپ ارسال کنید.", reply_markup=main_keyboard())
    else:
        kb = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="✅ شروع عضویت")]], resize_keyboard=True)
        await message.answer(f"سلام {message.from_user.first_name} عزیز 🌟\nبرای استفاده از ربات ابتدا باید ثبت‌نام کنید تا بتوانید عکس و کلیپ خود را برای ما ارسال کنید.", reply_markup=kb)

@dp.message(F.text == "✅ شروع عضویت")
...
async def ask_instagram(message: Message, state: FSMContext):
    ...
    await state.set_state(RegisterStates.instagram)
    await message.answer("آیدی پیج اینستاگرام خود را وارد کنید:")

@dp.message(RegisterStates.instagram)
async def ask_phone(message: Message, state: FSMContext):
    ...
    await message.answer("لطفاً شماره موبایل واقعی خود را از طریق دکمه زیر ارسال کنید:", reply_markup=kb)
...
async def complete_registration(message: Message, state: FSMContext):
    ...
    await message.answer("✅ ثبت‌نام با موفقیت انجام شد! اکنون می‌توانید عکس یا کلیپ ارسال کنید.", reply_markup=main_keyboard())
...
async def handle_upload(message: Message):
    ...
    if user_id not in users_data:
        return await message.answer("برای ارسال فایل ابتدا ثبت‌نام کنید.")
...
async def show_user_uploads(message: Message):
    user_id = str(message.from_user.id)
    if user_id not in users_data:
        return await message.answer("برای استفاده از این گزینه ابتدا ثبت‌نام کنید.")
...
@dp.message(F.text.startswith("/show_"))
